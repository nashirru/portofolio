    ...
    <section id="skills" className="py-20 relative overflow-hidden z-10">
      ...
      <div className="max-w-[1280px] mx-auto px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          >
            Mengkombinasikan keandalan arsitektur backend dengan keindahan antarmuka frontend yang modern.
          </motion.p>
        </div>
        ...
        >
          {skillsData.map((category, catIndex) => {
            ...
            return (
              ...
              >
                <div className="h-full p-8 flex flex-col justify-between transition-all duration-300 border border-glass-border bg-glass-fill backdrop-blur-xl rounded-glass-xl hover:border-accent-cyan/50 hover:bg-white/[0.02]">
                  <div>
                    {/* Category Header */}
                    <div className="flex items-center gap-4 mb-6">
                      <div className={`w-12 h-12 rounded-xl border flex items-center justify-center bg-white/[0.02] border-white/10 text-accent-cyan`}>
                      ...
                      {category.skills.map((skill, sIndex) => (
                        <div key={sIndex} className="space-y-2">
                          <div className="flex items-center justify-between text-xs sm:text-sm">
                            <span className="text-[#FAFAFA] font-medium">{skill.name}</span>
                            <span className="text-[#A1A1AA] font-mono text-xs">{skill.level}</span>
                          </div>
                          ...
                          <div className="h-1.5 w-full rounded-full bg-white/5 overflow-hidden">
                            <motion.div
                              ...
                              viewport={{ once: true }}
                              transition={{ duration: 1, delay: 0.1 + sIndex * 0.05, ease: "easeOut" }}
        ...
        <div className="mt-16 relative overflow-hidden py-4 border-y border-glass-border bg-white/[0.01] rounded-lg">
          ...
          <div className="flex whitespace-nowrap animate-marquee">
            {Array(2).fill(techLogos).flat().map((tech, tIndex) => (
              <span 
                key={tIndex} 
                className="text-[#A1A1AA]/45 font-mono font-bold text-xs sm:text-sm tracking-widest uppercase mx-10 inline-block transition-colors hover:text-accent-cyan"
              >
                // {tech}
              </span>
            ))}
