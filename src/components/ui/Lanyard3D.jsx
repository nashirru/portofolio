...
// Use the imported lanyard PNG asset URL as a fallback image to bypass texture loader crashes on base64 URIs
const BLANK_PIXEL = lanyard;
...
function Band({
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current && j1.current && j2.current && j3.current && card.current) {
      ...
      if (hasNaN || !t3 || isNaN(t3.x) || !tf || isNaN(tf.x) || !cardTrans || isNaN(cardTrans.x)) {
        return;
      }

      const j2Lerped = j2.current.lerped;
      const j1Lerped = j1.current.lerped;
      if (j2Lerped && j1Lerped && !isNaN(j2Lerped.x) && !isNaN(j1Lerped.x)) {
